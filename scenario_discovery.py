...
# filtered_experiments = chp_experiments
# filtered_outcomes = chp_outcomes

#Figure out what plants have above 300 kt/yr
grouped = chp_outcomes.groupby('Name')
gross_means = grouped['gross'].mean().sort_values()
# high_gross_names = gross_means[(gross_means > 250)].index.tolist()
# high_gross_names = gross_means[(gross_means < 125)].index.tolist()
...
# Define X and Y
x = filtered_experiments.iloc[:, 0:24]
# names_list = ['celc', 'i', 'heat_pump']  # This is essentially "constrained PRIM"
# x = x[names_list]
y = (filtered_outcomes["capture_cost"] < 120) & (filtered_outcomes["penalty_services"] < 450) & (filtered_outcomes["penalty_biomass"] < 1)
