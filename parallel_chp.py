...
#     if row['duration_increase'] == 2000 and row['heat_pump'] == True:  
#         return viridis(0.05), 1
#     elif row['duration_increase'] == 2000 and row['heat_pump'] == False:  
#         return viridis(0.0), 0.1
#     if row['duration_increase'] == 1000 and row['heat_pump'] == True:  
#         return viridis(0.5), 1
#     elif row['duration_increase'] == 1000 and row['heat_pump'] == False:  
#         return viridis(0.45), 0.1
#     if row['duration_increase'] == 0 and row['heat_pump'] == True:  
#         return viridis(0.80), 1
#     elif row['duration_increase'] == 0 and row['heat_pump'] == False:  
#         return viridis(0.75), 0.03
#     else:
#         return "grey", 0.4
def get_color(row):
    if row['duration_increase'] == 2000 and row['heat_pump'] == True:  
        ...
        return "crimson", 0.1
    elif row['duration_increase'] == 1000 and row['heat_pump'] == True:  
        return "deepskyblue", 1
    elif row['duration_increase'] == 1000 and row['heat_pump'] == False:  
        return "deepskyblue", 0.1
    elif row['duration_increase'] == 0 and row['heat_pump'] == True:  
        return "mediumseagreen", 1
    elif row['duration_increase'] == 0 and row['heat_pump'] == False:  
        return "mediumseagreen", 0.03
    else:
        return "grey", 0.4
...
for i, column in enumerate(data_scaled.columns):
    if column in numerical_columns:
        ...
        for pos, val in zip(tick_positions, tick_values):
            ax.text(i-0.1, pos, f"{int(round(val))}", ha='center', va='center', fontsize=10, color='black') #700
...
plt.savefig("parallel_chp.png", dpi=600, bbox_inches='tight')
plt.show()
